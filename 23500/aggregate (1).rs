...
impl Aggregator {
    ...
    /// # Arguments
    ///
    /// * `agg_fields` - List of `AggregateField`s to aggregate over. 
    /// *`AggregateField`s contains the aggregation function and the field to aggregate over.
    ...
    pub fn merge_tuple_into_group(&mut self, tuple: &Tuple) {
        ...
        if !self.hash_group.contains_key(&group_name) {
            ...
            self.hash_group.insert(group_name.clone(), agg_field.clone());
